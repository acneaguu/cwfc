...
for i = 2:length(Results_opt)
        ...
        if Results_no_opt(i).Times_converged >= 5
            ...
            costs_with_opt(i-1) = Results_opt(i).best_run_fitness;
            costs_with_optpar(i-1) = Results_opt_tuned(i).best_run_fitness;
            cost_turbinelevel(i-1) = Results_turbinelevel(i).best_run_fitness;
...
plot(cases(1:24),cost_turbinelevel(1:24),'-o','Color','blue','MarkerSize',3); 
ylabel('Costs [€]');
