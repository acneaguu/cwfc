function [violation_vec, total_violations] = compute_violation_constraints(PFresults)
global CONSTANTS Qref pen Casedata;
    ...
    violation_vbus_max = pen.p1*ones(Casedata.Nbus,1) - (PFresults.bus(:,CONSTANTS.VM) <= PFresults.bus(:,CONSTANTS.VMAX));  %vmax
    violation_vbus_min = pen.p1*ones(Casedata.Nbus,1) - (PFresults.bus(:,CONSTANTS.VM) >= PFresults.bus(:,CONSTANTS.VMIN));  %vmax
 
    %% Compute Qref violation
    %%check if Q at PCC is near Qref within the range given by tolerance.
    %%If not, 'vQpcc' is 1
    
    violation_Qpcc = pen.p2*(1 - (abs(Qpcc - Qref.setpoint) <= Qref.tolerance));
    ...
    violation_sbranchFrom = pen.p3*ones(Casedata.Nbranch,1) - (sbranchFrom <= PFresults.branch(:,CONSTANTS.RATE_A));
    ...
    violation_sbranchTo = pen.p3*ones(Casedata.Nbranch,1) - (sbranchTo <= PFresults.branch(:,CONSTANTS.RATE_A));
