...
global CONSTANTS Qref Optimisation Casedata;
    ...
    violation_vbus_max = Optimisation.p1*ones(Casedata.Nbus,1) - (PFresults.bus(:,CONSTANTS.VM) <= PFresults.bus(:,CONSTANTS.VMAX));  %vmax
    violation_vbus_min = Optimisation.p1*ones(Casedata.Nbus,1) - (PFresults.bus(:,CONSTANTS.VM) >= PFresults.bus(:,CONSTANTS.VMIN));  %vmax
    ...
    violation_sbranchFrom = Optimisation.p3*ones(Casedata.Nbranch,1) - (sbranchFrom <= PFresults.branch(:,CONSTANTS.RATE_A));
    ...
    violation_sbranchTo = Optimisation.p3*ones(Casedata.Nbranch,1) - (sbranchTo <= PFresults.branch(:,CONSTANTS.RATE_A));
