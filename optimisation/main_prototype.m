...
%%Load the data for prototype simulations
load('APC_workspace.mat') 
P_available_WTG = P_strings(:,1:13);
P_available_PVG = P_strings(:,14:end);
...
Qref.setpoint = [-0.143; 0; 0.143;0.143];
...
%         end
        ...
        %%BEUNIMPLEMENTATIE PROTOTYPE: Gebruikt geen windspeeds en
        %%irradiances om topologie te initialiseren en boundaries uit te
        %%rekenen
        Systemdata.mpc.gen(Systemdata.wtg_pos,CONSTANTS.PG) = P_available_WTG(j-1,:);
        Systemdata.mpc.gen(Systemdata.pvg_pos,CONSTANTS.PG) = P_available_PVG(j-1,:);
        
        
        [Qmax_wtg] = compute_QboundariesfromPWTG(P_available_WTG(j-1,:));
        [Qmax_pvg] = compute_QboundariesfromPPVG(P_available_PVG(j-1,:));
        Qmin_wtg = -Qmax_wtg;
        Qmin_pvg = 0*Qmax_pvg;
        %%%%%%%%%%%%%%%%%%EIND BEUNIMPLEMENTATIE%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             = compute_results(Results(j).Xbest(i+1,:),cases(j-1,2));
        [Results(j).Ploss(i+1), Results(j).Tap_changes(i+1), Results(j).Reactors_changes(i+1)...
            ,Results(j).extremeness_setpoints(i+1), Results(j).total_cost_per_run(i+1), Results(j).Qaccuracy(i+1)]...
            = compute_results(Results(j).Xbest(i+1,:),Qref.setpoint(j-1));
